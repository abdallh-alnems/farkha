...
class CycleExpensesController extends GetxController {
  ...
  String _getExpensesHash(List<dynamic> expenses) {
    ...
    return expenses.map((e) => '${e['id']}_${e['label']}_${e['value']}').join('|');
  }
  ...
  void _loadDefaultExpenses() {
    if (expenses.isEmpty) {
      expenses.addAll([
        ExpenseItem(id: 'chickens', label: 'الكتاكيت', icon: Icons.receipt),
        ExpenseItem(id: 'feed', label: 'العلف', icon: Icons.grain),
        ExpenseItem(
          id: 'medication',
          label: 'الأدوية والتحصينات',
          icon: Icons.medication,
        ),
        ExpenseItem(id: 'electricity', label: 'الكهرباء', icon: Icons.bolt),
        ExpenseItem(id: 'water', label: 'المياه', icon: Icons.water_drop),
        ExpenseItem(id: 'transport', label: 'النقل', icon: Icons.receipt),
        ExpenseItem(id: 'labor', label: 'العمالة', icon: Icons.people),
      ]);
    }
  }
  ...
  void _loadExpensesFromApi(List<dynamic> expensesFromApi) {
    ...
    for (var expense in expensesFromApi) {
      ...
      expensesMap[label]!.add({
        'id': expense['id']?.toString() ?? DateTime.now().millisecondsSinceEpoch.toString(),
        'amount': amount,
    ...
    for (var expense in expenses) {
      ...
      if (paymentsFromApi != null && paymentsFromApi.isNotEmpty) {
        ...
        final apiPayments = paymentsFromApi.map((p) => ExpensePayment(
          id: p['id'] as String,
          amount: p['amount'] as double,
          date: DateTime.tryParse(p['date'] as String) ?? DateTime.now(),
        )).toList();
        ...
        for (var localPayment in expense.payments) {
          ...
          if (localPayment.id.startsWith('payment_')) {
            ...
            for (var apiPayment in apiPayments) {
              final amountMatch = (apiPayment.amount - localPayment.amount).abs() < 0.01;
              final dateDiff = apiPayment.date.difference(localPayment.date).abs();
    ...
    expensesMap.forEach((label, payments) {
      if (!existingLabels.contains(label)) {
        ...
        final newItem = ExpenseItem(
          ...
          label: label,
          icon: icon,
          payments: payments.map((p) => ExpensePayment(
            id: p['id'] as String,
            amount: p['amount'] as double,
            date: DateTime.tryParse(p['date'] as String) ?? DateTime.now(),
          )).toList(),
  ...
  IconData _getIconForLabel(String label) {
    ...
    if (labelLower.contains('كتاكيت') || labelLower.contains('chicken')) {
      ...
      return Icons.grain;
    } else if (labelLower.contains('دواء') || labelLower.contains('تحصين') || labelLower.contains('medication')) {
      return Icons.medication;
    } else if (labelLower.contains('كهرباء') || labelLower.contains('electricity')) {
      return Icons.bolt;
  ...
  void _updateCycleTotalExpenses() {
    try {
      ...
      final currentTotalStr = cycleCtrl.cycles[idx]['total_expenses']?.toString() ?? '0';
