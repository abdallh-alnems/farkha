...
## Database Schema (Post-006 Overhaul)
- `cycle_data` uses EAV pattern: `metric_type` ENUM (weight/mortality/feed/vaccination/other) + `numeric_value` + `text_value` (replaces old `label`/`value`)
